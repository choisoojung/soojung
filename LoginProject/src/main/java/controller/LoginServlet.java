...
public class LoginServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	//2. 전달된 쿠키중에 idCookie와 passwordCookie를 찾는다.
    	String memberId = "";
    	...
    	//비지니스 로직은 컨트롤러에 구현하면 안됨
    			...
    			if(loginMember != null) {
    				...
    				//application : 동일 애플리케이션 에서는 모두 공유
    				...
    				//1.디스패치
    				//페이지 전환이 서버상에서 이루어짐
    				//주소 표시줄상의 요청 URL이 변경되지 않음 죽, /login 으로 되어있음
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cookieUse != null) {
			...
			//Cookie 객체를 생성하면 기본 생존기간은 -1로 설정됨
			//즉, 브라우저가 실행되어 있을 때는 쿠키가 존재하지만 브라우저를 종료하면 즉시 소멸됨
			//따라서, 생존기간을 명시적으로 설정해야 함
			idCookie.setMaxAge(24 * 60 * 60);//단위는 초
			...
			//즉, 브라우저가 실행되어 있을 때는 쿠키가 존재하지만 브라우저를 종료하면 즉시 소멸됨
			//따라서, 생존기간을 명시적으로 설정해야 함
			passwordCookie.setMaxAge(24 * 60 * 60);//단위는 초
